...
```bash
# 1. Refresh talent tree data from raidbots (talentIdMap.json)
#    MUST run BEFORE generateSpellsData — generateSpellsData reads talentIdMap.json
#    to discover every `type: "active"` talent spellId, so new-season talents
#    get CD metadata automatically.
cd <repo-root> && node scripts/update_statics.js

# 2. Regenerate spell ID lists (spellIdLists.json)
...
# 3. Regenerate spell effects data (spellEffects.json)
#    Reads talentIdMap.json (step 1) + spellIdLists.json (step 2) + spells.json
...
# 4. Regenerate PvP trinket item IDs (trinketItemIds.json)
...
# 5. Regenerate talent modifier extraction (talentModifiers.json)
#    Reads talentIdMap.json + spellEffects.json
cd <repo-root> && WAGO_BUILD=<LATEST_BUILD> npx ts-node --files --project packages/tools/tsconfig.json packages/tools/src/generateTalentModifiers.ts

# 6. Regenerate spell class map
cd <repo-root> && WAGO_BUILD=<LATEST_BUILD> npx ts-node --files --project packages/tools/tsconfig.json packages/tools/src/generateSpellClassMap.ts
```
