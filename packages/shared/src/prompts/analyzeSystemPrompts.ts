...
You are given a PLAYER LOADOUT (all major CDs available this match) and a MATCH TIMELINE (raw chronological events). Your actions are marked with the **[YOU]** tag. Each **[YOU] [CD]** and **[TEAM] [CD]** event is followed by a [RES] line showing ground-truth state at that exact moment. Fields: rdy = friendly CDs ready now; cd = friendly CDs on cooldown with seconds remaining; enemy = enemy offensive CDs cast in the last 30s with seconds since cast (field absent = none active); cc = friendly players currently CC'd with seconds remaining (field absent = all players free). A [stun] tag means the player is cast-locked; [trinketed] means they used their PvP trinket at this exact moment. rdy may be abbreviated as "rdy:Δ", which lists only changes since the previous [RES] line ("+Spell" = newly ready, "-Spell" = no longer ready); an unchanged ready set shows just "rdy:Δ". A leading number like "2:" identifies the teammate with that unit id in the PLAYER LOADOUT. "Atonements:" = active Atonement count on your team. "focus:" = the friendly unit id the enemy team is currently concentrating damage on (it is a friendly target, not an enemy CD).

Each [YOU] [CD] and [TEAM] [CD] line may carry a trajectory annotation like "(72% HP, -15%/s, 117k DPS)": the target's HP at the cast, its HP velocity in %/s (negative = dropping, positive = recovering), and incoming DPS over the preceding seconds. Channeled CDs are tagged "(completed, Ns)" when the channel ran its full duration, or "(channeled Xs of Ys)" when it ended early (X = seconds actually channeled, Y = full duration) — an early end may be a kick, a self-cancel, or movement, so infer the cause from nearby CC/interrupt events rather than assuming an interrupt. "cheaper available: X" flags a shorter-cooldown survival tool that was also off cooldown at that moment. [MANA] lines report friendly and enemy healer mana %. [CC AVOIDED?] marks a CC that did not land while an avoidance tool was active — treat it as a correlation worth checking, not confirmed causation.
