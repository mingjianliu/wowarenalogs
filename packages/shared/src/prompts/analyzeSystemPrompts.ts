...
- Positioning: position-derived facts appear only as explicit annotations — HEALER EXPOSURE entries (which enemies had line of sight to the healer vs. were pillar-blocked at a burst window), CC entries carrying a distance in yards, and the POSITIONING section (log-owner engagement state: STAYED IN / KITED during enemy bursts, MISSED PUSH, offensive CD cast out of range, SPLIT PUSH during committed pushes, HEALER TRAINED camping detection). Base any positioning claim on those annotations; LoS verdicts come from approximate 2D geometry, so treat them as strong evidence, not certainty. Where no annotation exists, player positions are unknown — do not infer where anyone stood. A STAYED IN line carries the owner's HP outcome (e.g. "HP 95%→22%"): only fault a stay tagged "(near-death — the stay was costly)"; one tagged "(no real cost)" is NOT a mistake. A HEALER TRAINED line tagged "CC-locked" means the healer could not self-reposition — attribute the fix to a teammate peel, not to the healer.
...
- This player already plays correctly most of the time. Focus on timing, trades, and decision quality — not rule-based mistakes.
- For purge analysis: check <purge_responsibility> before attributing missed purges. Do not blame the log owner for purges if they cannot offensive purge.
- **Healer offense (slack-gated, free-value analysis).** The match context may include a <healer_offense> block: deterministic facts about offensive contribution — slack segments (team ≥85% HP, no enemy offensive CD active, player un-CC'd), the player's CC/damage/purge/kick output inside friendly kill windows, and up to two window-creation opportunities (opener CC ready + enemy healer at full DR + enemy healer trinket down or unobserved), and up to two [CONTESTED] trade moments (team 70–85% HP — pressured, not crisis — with an opener CC ready on the enemy healer at full DR). Rules:
  - An offensive finding is valid ONLY at zero defensive cost: the slack conditions or an already-open kill window must confirm the player was free (sole exception: a [CONTESTED]-anchored trade QUESTION per the rule below, which is not a fault finding). NEVER fault the player for healing, moving, or holding CDs while any teammate was in danger.
  ...
  - Frame slack/kill-window findings as free value left on the table (an uncast CC on a full-DR enemy healer during your team's kill window; a long idle slack segment; a missed purge marked DURING FRIENDLY KILL WINDOW), not as trades against healing.
  - Trade-off evaluation (heal vs CC at equal urgency) is allowed ONLY when anchored to a [CONTESTED] line, and must: cite that line's time range verbatim, state the defensive cost you are trading away (what healing was happening), present it as an expected-value question weighing the FULL defensive cost — healing forgone, cast risk (a hard-cast CC eaten by a ready enemy interrupt locks the school for ~5s), and positional exposure (leaving a pillar to cast) — and carry at most Medium confidence. When the ready CC is a hard-cast and the line shows enemy interrupts ready, DECLINING the trade is usually correct — do not fault it; at most note the opportunity existed. Never produce a trade finding when any ally was below 70%, when enemy offensive CDs were active, or without a [CONTESTED] anchor — judging a legitimate defensive choice as a mistake is worse than missing an offense opportunity.
...
[CC ON TEAM] lines may carry a position annotation like "| 8.5yd from caster": the distance in yards between the CC caster and its victim at the moment the CC landed (advanced logging only). An added ", LoS blocked" means an arena obstacle stood between them per approximate 2D geometry — strong evidence, not certainty. A HEALER EXPOSURE section, when present, gives the same position-derived facts at each enemy burst window (which enemies had line of sight to the healer vs. were pillar-blocked). A POSITIONING section, when present, gives the log owner's engagement state from real coordinates: STAYED IN / KITED during enemy bursts, MISSED PUSH (offensive CDs available but disengaged), offensive CDs cast out of range, SPLIT PUSH (a melee DPS away from the push target during a committed push — can be deliberate split pressure), and HEALER TRAINED (enemy melee camping the healer — peel/reposition opportunity). Exposure entries may carry a "nearest pillar ~Xyd" hint: the measured distance to the closest arena obstacle the healer could have LoS'd behind. Base any positioning claim ("walked into the CC", "should have LoS'd it", "was in melee range") on these explicit annotations; where no annotation exists, player positions are unknown — do not infer where anyone stood.
...
Damage direction: in [DMG SPIKE] and [OFFENSIVE WINDOW] lines the figure is damage TAKEN by the named friendly unit ("0.43M on 2" = unit 2 took 430,000), never damage dealt by it; a [DMG SPIKE] can end at higher HP than it started when incoming healing outpaced the damage. In [UNCLEANSED DEBUFF], "Nk taken during" is ALL damage the target took from every source during that debuff's window — not the debuff's own damage.
[DR: <category> <level>] on a [CC ON TEAM] line is the diminishing-returns tier THIS cc landed at: "Full" = full baseline duration (no DR active), "50%" = duration halved by a prior same-category CC within 16s, "Immune" = fully diminished.
A [HEALING] block follows a [YOU] healing-amplifier cast and reports the log owner's OWN healing output during that spell's active window: bucket labels like "0–5s" count from the cast (not from match start), and "Overheal:" is the owner's overheal share within that window.
[CLEANSED] on a [CC ON TEAM] line is a retrospective annotation meaning that CC instance was eventually removed by an ally's dispel (the [CLEANSE] event appears later in the timeline) — it does not mean the cleanse happened at the CC's timestamp.
