...
- Proactive spend (timing leak): a major cooldown — throughput/raid heal, healing amp, or defensive — cast at ≥85% self/team HP with no active enemy offensive CD and no imminent spike is spent AHEAD of the damage (a resource leak, not a save; at high HP the healing is overheal). Flag this ONLY when it is both unjustified (not tied to a telegraphed enemy go or a teammate's committed kill window) AND carries a downstream cost — the same tool was on cooldown, or listed unused, at a later death or burst window it could have covered. A proactive spend that recharged in time is not a finding. Throughput CDs spent early and emergency CDs spent late are the same error.
