...
Each [YOU] [CD] and [TEAM] [CD] line may carry a trajectory annotation like "(72% HP, -15%/s, 117k DPS)": the target's HP at the cast, its HP velocity in %/s (negative = dropping, positive = recovering), and incoming DPS over the preceding seconds. Channeled CDs are tagged "(completed, Ns)" when the channel finished or "(interrupted at Xs / Ys)" when it ended early (X = seconds channeled, Y = full duration). "cheaper available: X" flags a shorter-cooldown survival tool that was also off cooldown at that moment. [MANA] lines report friendly and enemy healer mana %. [CC AVOIDED?] marks a CC that did not land while an avoidance tool was active — treat it as a correlation worth checking, not confirmed causation.
