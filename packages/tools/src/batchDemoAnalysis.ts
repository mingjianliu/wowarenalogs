...
async function runDemoForSpec(specToTest: string) {
  ...
  const metricLines = lines.slice(lines.indexOf('### Global Metric Gaps:'), lines.indexOf('### User\'s Crisis Responses (<40% HP events):'));
  ...
  if (analysisData.userCrisisEvents.length > 0 && analysisData.nearestNeighbors[0].crisisEvents.length > 0) {
      console.log(`User at ${analysisData.userCrisisEvents[0].split(':')[0]}: ${analysisData.userCrisisEvents[0].split(':')[1]}`);
      console.log(`Pro at ${analysisData.nearestNeighbors[0].crisisEvents[0].split(':')[0]}: ${analysisData.nearestNeighbors[0].crisisEvents[0].split(':')[1]}`);
  }
}
